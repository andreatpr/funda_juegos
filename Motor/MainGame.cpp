...
void MainGame::initLevel() {
	...
	std::mt19937 randomEnginez(time(nullptr));
	std::uniform_int_distribution<int>randPosXZ(1, levels[currentLevel]->getWidth() - 2);
	std::uniform_int_distribution<int>randPosYZ(1, levels[currentLevel]->getHeight() - 2);
	...
	for (int i = 0; i < levels[currentLevel]->getNumZombies(); ++i) {
		...
		glm::vec2 pos(randPosXZ(randomEnginez) * TILE_WIDTH,
			randPosYZ(randomEnginez) * TILE_WIDTH);
...
void MainGame::draw() {
	...
	for (size_t i = 0; i < zombies.size(); i++) {
		zombies[i]->draw(spriteBatch);//aparezca
	}
	spriteBatch.end();
